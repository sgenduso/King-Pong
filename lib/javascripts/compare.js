...
var avgPtDiff = function (ptDiffObj) {
  var leaders = {};
  console.log(ptDiffObj);
  for(var name in ptDiffObj) {
    var total = ptDiffObj[name].length;
    ptDiffObj[name] = ptDiffObj[name].reduce(function (a,b) {
      return a + b;
    });
    leaders[name] = (ptDiffObj[name] / total).toFixed(0);
  }
  console.log(leaders);
};
