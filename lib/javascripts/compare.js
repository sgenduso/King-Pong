...
//returns avg pt diff of winning games by each
var avgPtDiff = function (ptDiffObj) {
  var leaders = {};
  var newPtObj =(JSON.parse(JSON.stringify(ptDiffObj)));
  for(var name in newPtObj) {
    var total = newPtObj[name].length;
    if (total > 0) {
    newPtObj[name] = newPtObj[name].reduce(function (a,b) {
      return a + b;
    });
    leaders[name] = (newPtObj[name] / total).toFixed(0);
    }
  }
  return leaders;
};
